...
void sprite_flush(pSprite s)
{
    ...
    if(s->nr_of_shots > 0)
    {
        ...
        int buflen = snprintf(NULL, 0, "%s_vtt_%d%s", s->filenamebase, s->curr_file_idx, gb_o_suffix) + sizeof(char);
        char *outname = (char*)malloc(buflen);
        snprintf(outname, buflen, "%s_vtt_%d%s", s->filenamebase, s->curr_file_idx, gb_o_suffix);
...
int sprite_export_vtt(pSprite s)
{
    ...
    sprintf(outname, "%s.vtt", s->filenamebase);
